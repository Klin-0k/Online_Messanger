...
class ChangeProfileForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        ).exists() and self.current_user.UserName != cleaned_data.get("UserName"):
            self.add_error("UserName", "This username is already taken!")
        if User.objects.filter(
            Email=cleaned_data.get("Email")
        ).exists() and self.current_user.Email != cleaned_data.get("Email"):
            self.add_error("Email", "User with this Email already exists!")
        if (
            cleaned_data.get("CurrentPassword")
            or cleaned_data.get("NewPassword")
            or cleaned_data.get("ConfirmPassword")
        ):
            if self.current_user.Password != cleaned_data.get("CurrentPassword"):
